...
private:
    ...
    NodePtr statement() {
        if (current.type == Lexer::TOK_IDENT) {
            ...
            if (current.text == "repeat") return parse_repeat();
            if (current.text == "break") return parse_break();
    ...
    NodePtr parse_repeat() {
        advance();
        auto node = std::make_shared<Node>(Node::NODE_REPEAT);
        node->children.push_back(parse_block_until({"until"}));
        if (!(current.type == Lexer::TOK_IDENT && current.text == "until")) {
            throw std::runtime_error("expected 'until'");
        }
        advance();
        node->children.push_back(logical_or());
        return node;
    ...
    NodePtr parse_break() {
        advance();
        return std::make_shared<Node>(Node::NODE_BREAK);
    }
    ...
    NodePtr logical_and() {
        ...
        while (current.type == Lexer::TOK_IDENT && current.text == "and") {
            ...
            node->text = "and";
            node->children.push_back(left);
            node->children.push_back(bitwise_or());
            left = node;
        }
        return left;
    }
    ...
    NodePtr bitwise_or() {
        ...
        while (current.type == Lexer::TOK_PIPE) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
            node->children.push_back(left);
            node->children.push_back(bitwise_xor());
            left = node;
        }
        return left;
    }
    ...
    NodePtr bitwise_xor() {
        ...
        while (current.type == Lexer::TOK_TILDE) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
            node->children.push_back(left);
            node->children.push_back(bitwise_and());
            left = node;
        }
        return left;
    }
    ...
    NodePtr bitwise_and() {
        auto left = equality();
        while (current.type == Lexer::TOK_AMP) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
    ...
    NodePtr shift() {
        auto left = concat();
        while (current.type == Lexer::TOK_SHL || current.type == Lexer::TOK_SHR) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
            node->children.push_back(left);
            node->children.push_back(concat());
            left = node;
        }
        return left;
    }
    ...
    NodePtr concat() {
        ...
        if (current.type == Lexer::TOK_CONCAT) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
    ...
    NodePtr term() {
        ...
        while (current.type == Lexer::TOK_STAR ||
               ...
               current.type == Lexer::TOK_DSLASH ||
               current.type == Lexer::TOK_PERCENT) {
    ...
    NodePtr unary() {
        ...
        if (match(Lexer::TOK_HASH)) {
            auto node = std::make_shared<Node>(Node::NODE_UNARY);
            node->text = "#";
            node->children.push_back(unary());
            return node;
        }
        if (match(Lexer::TOK_TILDE)) {
            auto node = std::make_shared<Node>(Node::NODE_UNARY);
            node->text = "~";
            node->children.push_back(unary());
            return node;
    ...
    NodePtr power() {
        auto left = postfix();
        if (current.type == Lexer::TOK_CARET) {
            std::string op = current.text;
            advance();
            auto node = std::make_shared<Node>(Node::NODE_BINARY);
            node->text = op;
...
public:
    ...
    std::map<std::string, void*> c_symbol_table;
    std::map<std::string, std::string> cpp_symbol_table;
    std::map<std::string, ValuePtr> podlet_cache;
...
public:
    ...
    ValuePtr eval(const NodePtr& node) {
        switch (node->type) {
            ...
            case Node::NODE_REPEAT: {
                ValuePtr out = std::make_shared<Value>();
                do {
                    push_scope();
                    out = eval(node->children[0]);
                    const bool should_break = break_flag;
                    if (should_break) {
                        break_flag = false;
                    }
                    const bool done = should_break || eval(node->children[1])->truthy();
                    pop_scope();
                    if (return_flag) return out;
                    if (done) break;
                } while (true);
    ...
    bool load_stdc(const std::string& path) {
        return load_sources_from_directory(path.empty() ? default_stdc_root() : path, ".c", false);
    }

    bool load_stdcpp(const std::string& path) {
        const std::string root = path.empty() ? default_stdcpp_root() : path;
        bool ok = true;
        ok = linker.add_file(root + "/QaMRpp-Library.hpp") && ok;
        ok = load_sources_from_directory(root, ".hpp", true) && ok;
        return ok;
    }

    void register_c_symbol(const std::string& name, void* fn_ptr) {
        if (!name.empty() && fn_ptr) {
            c_symbol_table[name] = fn_ptr;
        }
    }

    template <typename T>
    void register_cpp_symbol(const std::string& name, T* symbol_ptr) {
        if (name.empty() || symbol_ptr == nullptr) {
            return;
        }
        cpp_symbol_table[name] = typeid(T).name();
    }
...
private:
    ...
    static std::string strip_qpod_suffix(const std::string& value) {
        if (has_suffix(value, ".qpod")) {
    ...
    void set_runtime_error(const std::string& message) {
        qamrpp_set_error(reinterpret_cast<qamrpp_context*>(this), QAMRPP_ERR_GENERIC, message.c_str());
    }

    void table_set_string_field(const ValuePtr& table, const std::string& key, const std::string& value) {
        table_raw_set(table, std::make_shared<Value>(key), std::make_shared<Value>(value));
    }

    bool materialize_podlet_exports(
        const std::map<std::string, std::string>& manifest,
        const std::string& source_payload
    ) {
        auto get_required = [&](const char* key, std::string& out) -> bool {
            auto it = manifest.find(key);
            if (it == manifest.end() || it->second.empty()) {
                set_runtime_error(std::string("podlet manifest missing required key: ") + key);
                return false;
            }
            out = it->second;
            return true;
        ...
        if (!get_required("name", name)) return false;
        if (!get_required("version", version)) return false;
        if (!get_required("entrypoint", entrypoint)) return false;

        const std::string cache_key = name + "@" + version;
        auto cache_it = podlet_cache.find(cache_key);
        if (cache_it != podlet_cache.end()) {
            assign_name(name, cache_it->second);
            return true;
        }

        ValuePtr exports = make_table();
        table_set_string_field(exports, "name", name);
        table_set_string_field(exports, "version", version);
        table_set_string_field(exports, "entrypoint", entrypoint);
        table_set_string_field(exports, "source", source_payload);

        ValuePtr manifest_table = make_table();
        for (std::map<std::string, std::string>::const_iterator it = manifest.begin(); it != manifest.end(); ++it) {
            table_set_string_field(manifest_table, it->first, it->second);
        }
        table_raw_set(exports, std::make_shared<Value>(std::string("manifest")), manifest_table);

        podlet_cache[cache_key] = exports;
        assign_name(name, exports);
        return true;
    }
    ...
    bool load_podlet_archive_from_path(const std::string& path, const std::string& requested_name) {
        ...
        std::map<std::string, std::string>::const_iterator name_it = manifest.find("name");
        if (name_it == manifest.end() || name_it->second.empty()) {
            set_runtime_error("podlet manifest missing required key: name");
            return false;
        }

        if (!requested_name.empty()) {
            const std::string normalized_requested = strip_qpod_suffix(requested_name);
            const bool looks_like_path =
                normalized_requested.find('/') != std::string::npos ||
                normalized_requested.find('\\') != std::string::npos;
            if (!looks_like_path && normalized_requested != name_it->second) {
                set_runtime_error("podlet manifest name does not match requested package");
                return false;
            }
        }

        std::map<std::string, std::string>::const_iterator entry_it = manifest.find("entrypoint");
        if (entry_it == manifest.end() || entry_it->second.empty()) {
    ...
    bool load_podlet_named(const std::string& name, const std::vector<std::string>& roots) {
        ...
        if (has_suffix(name, ".qpod")) {
            ...
            for (size_t i = 0; i < roots.size(); ++i) {
                candidates.push_back(roots[i] + "/" + name);
            }
        } else {
            for (size_t i = 0; i < roots.size(); ++i) {
                candidates.push_back(roots[i] + "/" + name + ".qpod");
                candidates.push_back(roots[i] + "/podlet/" + name + ".qpod");
            }
        }
    ...
    bool load_sources_from_directory(const std::string& dir, const std::string& ext, bool skip_library_header) {
        if (!ensure_dir(dir)) {
            return false;
        }
        ...
        for (dirent* ent = readdir(d); ent != nullptr; ent = readdir(d)) {
            ...
            if (skip_library_header && name == "QaMRpp-Library.hpp") continue;
            if (name.size() < ext.size() || name.substr(name.size() - ext.size()) != ext) continue;
            ok = linker.add_file(dir + "/" + name) && ok;
        }
        closedir(d);
        return ok;
    }
    ...
    ValuePtr eval_binary(const NodePtr& node) {
        ...
        else if (op == "//") metamethod_name = "__idiv";
        else if (op == "%") metamethod_name = "__mod";
        else if (op == "^") metamethod_name = "__pow";
        else if (op == "..") metamethod_name = "__concat";
        else if (op == "&") metamethod_name = "__band";
        else if (op == "|") metamethod_name = "__bor";
        else if (op == "~") metamethod_name = "__bxor";
        else if (op == "<<") metamethod_name = "__shl";
        else if (op == ">>") metamethod_name = "__shr";
        ...
        if (op == "..") {
            const bool lhs_ok = lhs->type == Value::STRING || lhs->type == Value::INT || lhs->type == Value::FLOAT;
            const bool rhs_ok = rhs->type == Value::STRING || rhs->type == Value::INT || rhs->type == Value::FLOAT;
            if (!lhs_ok || !rhs_ok) {
                throw std::runtime_error("concatenation expects string or number operands");
            }
            return std::make_shared<Value>(lhs->to_string() + rhs->to_string());
        }
        ...
        if (!lhs_is_number || !rhs_is_number) {
            if ((op == "<" || op == "<=" || op == ">" || op == ">=") &&
                lhs->type == Value::STRING && rhs->type == Value::STRING) {
                if (op == "<") return std::make_shared<Value>(lhs->string_value < rhs->string_value);
                if (op == "<=") return std::make_shared<Value>(lhs->string_value <= rhs->string_value);
                if (op == ">") return std::make_shared<Value>(lhs->string_value > rhs->string_value);
                if (op == ">=") return std::make_shared<Value>(lhs->string_value >= rhs->string_value);
            }
            throw std::runtime_error("operation expects numeric operands");
        }

        auto to_int = [](const ValuePtr& value) -> int64_t {
            if (value->type == Value::INT) return value->int_value;
            if (value->type == Value::FLOAT && std::floor(value->float_value) == value->float_value &&
                value->float_value >= static_cast<double>(std::numeric_limits<int64_t>::min()) &&
                value->float_value <= static_cast<double>(std::numeric_limits<int64_t>::max())) {
                return static_cast<int64_t>(value->float_value);
            }
            throw std::runtime_error("bitwise operation expects integer operands");
        };
        ...
        if (op == "//") {
            ...
            if (lhs->type == Value::INT && rhs->type == Value::INT) {
                return std::make_shared<Value>(static_cast<int64_t>(q));
            }
            return std::make_shared<Value>(q);
        }

        if (op == "%") {
            double r = a - std::floor(a / b) * b;
            if (lhs->type == Value::INT && rhs->type == Value::INT) {
                return std::make_shared<Value>(static_cast<int64_t>(r));
            }
            return std::make_shared<Value>(r);
        }

        if (op == "^") {
            return std::make_shared<Value>(std::pow(a, b));
        }

        if (op == "&" || op == "|" || op == "~" || op == "<<" || op == ">>") {
            int64_t ia = to_int(lhs);
            int64_t ib = to_int(rhs);
            if (op == "&") return std::make_shared<Value>(ia & ib);
            if (op == "|") return std::make_shared<Value>(ia | ib);
            if (op == "~") return std::make_shared<Value>(ia ^ ib);
            const int width = static_cast<int>(sizeof(int64_t) * 8);
            if (ib < 0) {
                if (ib <= -width) return std::make_shared<Value>(static_cast<int64_t>(0));
                return std::make_shared<Value>(op == "<<" ? static_cast<int64_t>(static_cast<uint64_t>(ia) >> -ib)
                                                          : static_cast<int64_t>(static_cast<uint64_t>(ia) << -ib));
            }
            if (ib >= width) return std::make_shared<Value>(static_cast<int64_t>(0));
            return std::make_shared<Value>(op == "<<" ? static_cast<int64_t>(static_cast<uint64_t>(ia) << ib)
                                                      : static_cast<int64_t>(static_cast<uint64_t>(ia) >> ib));
    ...
    ValuePtr eval_unary(const NodePtr& node) {
        ...
        if (node->text == "#") {
            ValuePtr metamethod = lookup_metamethod(value, "__len");
            if (metamethod) {
                if (metamethod->type != Value::FUNCTION) {
                    throw std::runtime_error("__len metamethod is not callable");
                }
                std::vector<ValuePtr> args;
                args.push_back(value);
                return metamethod->function_value(*this, args);
            ...
            if (value->type == Value::STRING) {
                return std::make_shared<Value>(static_cast<int64_t>(value->string_value.size()));
            }
            if (value->type == Value::TABLE) {
                int64_t border = 0;
                while (true) {
                    ValuePtr next_key = std::make_shared<Value>(border + 1);
                    ValuePtr next_value = table_raw_lookup(value, next_key);
                    if (!next_value || next_value->type == Value::NIL) break;
                    ++border;
                }
                return std::make_shared<Value>(border);
            }
            throw std::runtime_error("length operator expects string or table");
        }
        if (node->text == "~") {
            if (value->type == Value::INT) {
                return std::make_shared<Value>(~value->int_value);
            ...
            if (value->type == Value::FLOAT && std::floor(value->float_value) == value->float_value) {
                return std::make_shared<Value>(~static_cast<int64_t>(value->float_value));
            }
            throw std::runtime_error("unary '~' expects integer operand");
        }
